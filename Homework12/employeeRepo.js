...
var orm = {
  ...
  addEmployee: async function(){
    try{
      let data = await orm.getTable("employee");
      ...
      await orm.getTable("role");
      const {roleId} = await inquirer.prompt(questions[2]);
      await orm.getTable("employee");
  ...
  addRole: async function(){
    try{
      let data = await orm.getTable("departments");
      ...
      await orm.getTable("departments");
  ...
  deleteItem: async function(){
    try{
      ...
      let data = await orm.getTable(toBeDeleted);
  ...
  updateItem: async function(){
    try{
      ...
      let data = await orm.getTable(toBeUpdated);
      let idChoices = await data.map(x => x.id);
